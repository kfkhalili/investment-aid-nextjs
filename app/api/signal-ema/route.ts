...
import {
  processEmaSignalsForSymbol,
  type EmaProcessingResult,
} from "@/lib/services/signal-ema/service";

export async function GET(): Promise<NextResponse> {
  console.log(
    "[API EmaSignal ALL] Received request to process all symbols for EMA signals."
  );
  ...
  try {
    ...
    if (symbolsError) {
      console.error(
        "[API EmaSignal ALL] Error fetching symbols from profile_symbols:",
    ...
    if (!symbolsData || symbolsData.length === 0) {
      console.log(
        "[API EmaSignal ALL] No symbols found in profile_symbols view."
      );
      return NextResponse.json(
    ...
    if (symbolsToProcess.length === 0) {
      console.log(
        "[API EmaSignal ALL] No valid symbols to process after filtering."
      );
      return NextResponse.json(
    ...
    console.log(
      `[API EmaSignal ALL] Found ${symbolsToProcess.length} symbols to process.`
    );
  } catch (e: unknown) {
    const errorMessage = e instanceof Error ? e.message : String(e);
    console.error(
      "[API EmaSignal ALL] Unexpected error fetching symbols:",
      errorMessage
    );
    return NextResponse.json(
      {
  ...
  const allResults: EmaProcessingResult[] = [];
  for (const symbol of symbolsToProcess) {
    // Pass the existing supabase client instance to the service function
    const result = await processEmaSignalsForSymbol(symbol, supabase);
  ...
  const responseSummary = {
    message: `EMA signals processing complete for all ${symbolsToProcess.length} attempted symbols.`,
  ...
  console.log(
    `[API EmaSignal ALL] Processing finished. Generated ${totalSignalsGenerated} signals. Errors: ${errorCount}.`
  );
