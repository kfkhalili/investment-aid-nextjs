...
import {
  processMacdSignalsForSymbol,
  type MacdProcessingResult,
} from "@/lib/services/signal-macd/service";

export async function GET(): Promise<NextResponse> {
  console.log(
    "[API MacdSignal ALL] Received request to process all symbols for MACD signals."
  );
  ...
  try {
    ...
    if (symbolsError) {
      console.error(
        "[API MacdSignal ALL] Error fetching symbols from profile_symbols:",
    ...
    if (!symbolsData || symbolsData.length === 0) {
      console.log(
        "[API MacdSignal ALL] No symbols found in profile_symbols view."
      );
      return NextResponse.json(
    ...
    if (symbolsToProcess.length === 0) {
      console.log(
        "[API MacdSignal ALL] No valid symbols to process after filtering."
      );
      return NextResponse.json(
    ...
    console.log(
      `[API MacdSignal ALL] Found ${symbolsToProcess.length} symbols to process.`
    );
  } catch (e: unknown) {
    const errorMessage = e instanceof Error ? e.message : String(e);
    console.error(
      "[API MacdSignal ALL] Unexpected error fetching symbols:",
      errorMessage
    );
    return NextResponse.json(
      {
  ...
  const allResults: MacdProcessingResult[] = [];
  for (const symbol of symbolsToProcess) {
    const result = await processMacdSignalsForSymbol(symbol, supabase);
  ...
  const responseSummary = {
    message: `MACD signals processing complete for all ${symbolsToProcess.length} attempted symbols.`,
  ...
  console.log(
    `[API MacdSignal ALL] Processing finished. Generated ${totalSignalsGenerated} signals. Errors: ${errorCount}.`
  );
